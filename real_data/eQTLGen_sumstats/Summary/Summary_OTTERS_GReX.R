...
sub.res.dir = 'RDA/Results'

# test sample
data.dir = '~/projects/OTTERS/Data'
# get samples that are in GTEx V8 but not in GTEx v6
V6.ID = read.table(file.path(data.dir, "RDA/GTEx_V6", "SampleID.txt"),
                   sep = '\t')
V8.ID = read.table(file.path(data.dir, "RDA/GTEx_V8", "SampleID.txt"),
                   sep = '\t')
Test.ID = V8.ID$V1[!V8.ID$V1 %in% V6.ID$V1]
...
GReX_Method = function(method){
  ...
  for (row in 1:nrow(jobs)){
    ...
    Pred.GReX = Pred.GReX[inter.genes, Test.ID]
    Sub.Exp.mat = Exp.mat[inter.genes, Test.ID]
